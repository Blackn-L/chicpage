...
const segmentedActiveClassName =
...
interface MobileTopBarProps {
  ...
  onCopy: () => void;
  onStyleThemeChange: (theme: "wechat" | "poster") => void;
...
function ThemeCard({
  ...
  const wechatTheme = "containerStyle" in theme && !("background" in theme)
    ? (theme as WechatTheme)
    : null;
  const posterTheme = "background" in theme ? (theme as PosterTheme) : null;
  const wechatBackground = wechatTheme
    ? getThemeBackgroundStyle(wechatTheme)
    : null;
  const wechatTextureLayer = wechatTheme
    ? getThemeTextureLayer(wechatTheme)
    : null;
        ...
        {wechatTextureLayer ? (
          <span
            ...
            style={{
              ...
              backgroundRepeat:
                wechatTheme?.id === "linedpaper2" ? "repeat" : "no-repeat",
...
export function MobileTopBar({
  ...
  return (
    <header className="shrink-0 border-b border-border bg-card/90 px-3 py-2.5 backdrop-blur-xl">
      {panel === "edit" ? (
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex min-w-0 flex-1 items-center justify-end gap-2 overflow-x-auto [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            <ModeSwitch
              ...
              onStyleThemeChange={onStyleThemeChange}
            />
            ...
            <Button
              ...
              className={cn(
                iconButtonClassName,
                showWordCount && activeIconButtonClassName,
              )}
            >
